...
export default function RightDrawer({ open, onClose, data }) {
  ...
  useEffect(() => {
    const getOwnerInfo = async () => {
      ...
      try {
        const res = await API.get(`/vehicle-owner-information/${data.email}`);
            ...
            <div>
              <div className="flex flex-col items-center mb-6">
                {info.profile_img ? (
                  <img
                    src={
                      info.profile_img?.startsWith("http")
                        ? info.profile_img
                        : `${IMAGE_BASE_URL}/uploads/${info.profile_img}`
                    }
                    alt="profile"
                    className="w-24 h-24 rounded-full object-cover border-4 border-indigo-500 shadow-lg"
                    onError={(e) => {
                      e.target.src =
                        "https://ready-player-me-status.s3.amazonaws.com/avatar-placeholder.png";
                    }}
                  />
                ) : (
